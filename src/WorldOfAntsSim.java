...
class Anteater extends Organism
{
	...
	public static final int ANTEATERBREED = 8;	// How many ticks to breed an anteater
	public static final int ANTEATERSTARVE = 3; // How many ticks for anteater to die
	protected int starveTicks;  //
	...
	* Anteater breed
	* Increment the tick count for breeding.
	* If it equals our threshold, then clone this anteater either
	* above, right, left, or below the current one.
	*/
	public void breed()			// Must define this since virtual
	{
	...
	/**
	* Anteater move
	* Look for an empty or Ant occupied cell up, right, left, or down and
	* try to move there.  If Anteater moves to ant occupied cell, reset starveTicks
	*/
	public void move()			// Must define this since virtual
	{
		// Pick random direction to move
		int dir = (int) (Math.random() * 4);
		// Try to move up, if not at an edge and not empty/Ant-occupied spot
		if (dir==0)
		{
			if ((y>0) && ((world.getAt(x,y-1)==null) || (world.getAt(x, y-1) instanceof Ant)))
			{
			 if (world.getAt(x, y-1) instanceof Ant)  //If Anteater eats Ant, reset StarveTicks
				 starveTicks = 0;
			 world.setAt(x,y-1,world.getAt(x,y));  // Move to new spot
			 world.setAt(x,y,null);
			 y--;
		...
		{
			if ((y<World.WORLDSIZE-1) && ((world.getAt(x,y+1)==null) || (world.getAt(x,y+1) instanceof Ant)))
			{
				if (world.getAt(x, y+1) instanceof Ant)  //If Anteater eats Ant, reset StarveTicks
					 starveTicks = 0;
				world.setAt(x,y+1,world.getAt(x,y));  // Move to new spot
		...
		{
			if ((x>0) && ((world.getAt(x-1,y)==null) || (world.getAt(x-1, y) instanceof Ant)))
			{
				 if (world.getAt(x-1, y) instanceof Ant)  //If Anteater eats Ant, reset StarveTicks
					 starveTicks = 0;
				 world.setAt(x-1,y,world.getAt(x,y));  // Move to new spot
				 world.setAt(x,y,null);  // Set current spot to empty
			 	 x--;
		...
		{
			if ((x<World.WORLDSIZE-1) && ((world.getAt(x+1,y)==null) || (world.getAt(x+1,y) instanceof Ant)))
			{
				 if (world.getAt(x+1, y) instanceof Ant)  //If Anteater eats Ant, reset StarveTicks
					 starveTicks = 0;
				 world.setAt(x+1,y,world.getAt(x,y));  // Move to new spot
	...
	/**
	* Return true when anteater starveTicks equals ANTEATERSTARVE value
	*/
	public boolean starve()
	{
		starveTicks++;  // When called increase starveTick count
		
		if (starveTicks <= ANTEATERSTARVE)  // Return if Anteater hit Starve value
		{
			return false;   
		}
		else 
